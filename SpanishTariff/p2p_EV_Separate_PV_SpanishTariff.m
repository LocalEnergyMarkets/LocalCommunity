function [linsol,fval]=p2p_EV_Separate_PV_SpanishTariff(dem,Pe,Pf,Nt,res,psip2p,ET3,PowerCost,TimeStepperHour, ATS, DTS, Nev, Cap_perEV, CHrate,DCHrate,arrSoC,DepSoC)
dem=[dem zeros(Nt,1)];
res=[res zeros(Nt,1)];
Nh=length(dem(1,:));
%% defining variables
...
%% Objective function
linprob = optimproblem('Objective',Pe'*(sum(G,2)) + Pf'*sum(sum(Xp2p,3),2) + TimeStepperHour*PowerCost'*Peak);
%% Constraints
...
for t=1:Nt
    if t<ats || t>dts
        linprob.Constraints.eq1EV=[linprob.Constraints.eq1EV,G(t,aa(2))+I(t,aa(2))>=dem(t,aa(2))+X(t,aa(2))];
    elseif t>=ats && t<=dts
        tt=tt+1;
        linprob.Constraints.eq1EV=[linprob.Constraints.eq1EV,G(t,aa(2))+I(t,aa(2))+EVD(tt,1)>=dem(t,aa(2))+X(t,aa(2))+EVC(tt,1)];
    %else
