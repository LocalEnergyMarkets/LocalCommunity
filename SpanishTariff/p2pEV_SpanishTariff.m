function [linsol,fval]=p2pEV_SpanishTariff(dem,Pe,Pf,Nt,res,psip2p,ET3,PowerCost,TimeStepperHour, ATS, DTS, Nev, Cap_perEV, CHrate,DCHrate,arrSoC,DepSoC)
Nh=length(dem(1,:));
%% defining variables
aa=size(dem);
...
for t=1:Nt
    ...
    elseif t>=ats && t<=dts
        tt=tt+1;
        linprob.Constraints.eq1EV=[linprob.Constraints.eq1EV,res(t,aa(2))+G(t,aa(2))+I(t,aa(2))+EVD(tt,1)>=dem(t,aa(2))+X(t,aa(2))+EVC(tt,1)];
    %else
    %    linprob.Constraints.eq1EV=[linprob.Constraints.eq1EV,res(t,aa(2))+G(t,aa(2))+I(t,aa(2))>=dem(t,aa(2))+X(t,aa(2))];
    end
end
