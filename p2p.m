function [linsol,fval]=p2p(dem,Pg,alpha,beta,Sl,Su,eta_ch,eta_dis,Nt,res,BatteryPlace,S0,psip2p,feed_in_tariff)
Nh=length(dem(1,:));
%% defining variables
aa=size(dem);
G=optimvar('G',aa(1),aa(2),'LowerBound',0);
Fi=optimvar('Fi',aa(1),aa(2),'LowerBound',0);
...
%% Objective function
linprob = optimproblem('Objective',Pg'*(sum(G,2))-feed_in_tariff*sum(sum(Fi)));
%% Constraints
...
%         linprob.Constraints.eq1=[linprob.Constraints.eq1,res(:,i)+G(:,i)+I(:,i)+D(:,k)>=dem(:,i)+X(:,i)+C(:,k)];
        linprob.Constraints.eq1=[linprob.Constraints.eq1,res(:,i)+G(:,i)+I(:,i)+D(:,k)==dem(:,i)+X(:,i)+C(:,k)+Fi(:,i)];
        
    else
%         linprob.Constraints.eq1=[linprob.Constraints.eq1,res(:,i)+G(:,i)+I(:,i)>=dem(:,i)+X(:,i)];
        linprob.Constraints.eq1=[linprob.Constraints.eq1,res(:,i)+G(:,i)+I(:,i)==dem(:,i)+X(:,i)+Fi(:,i)];
    end
end
