function [linsol,fval]=p2pEV(dem,res,Marginal_LR, Pg, En_price, Nt, EVPlace, EValpha, EVbeta, EVcap, ats,dts,arrivalSoC, DepartureSoC, eta_ch, eta_dis)
...
for i=1:Nh
    if EVPlace(i)==1
        kk=kk+1;
        linprob.Constraints.eq1=[linprob.Constraints.eq1,res(:,i)+G(:,i)+I(:,i)+EVD(:,kk)==dem(:,i)+X(:,i)+EVC(:,kk)+N(:,i)];
    else
        linprob.Constraints.eq1=[linprob.Constraints.eq1,res(:,i)+G(:,i)+I(:,i)==dem(:,i)+X(:,i)+N(:,i)];
    end
end
